...
def update_captcha(captcha, user, passwd):
    conn = psycopg2.connect(pyadmin.conn.conn)
    cur = conn.cursor()
    cur.execute("update account set captcha=%s where username=%s and account_password=%s", (captcha, user, passwd))
    conn.commit()
    cur.close()
    conn.close()
...
def application(environ, start_response):
    ...
    if not 'username' in post or post['username'] == '' or not 'password' in post \
        ...
        if res['success'] == True:
            ...
            update_captcha(post['g-recaptcha-response'], user, hashlib.sha512(passwd.encode('utf-8')).hexdigest())
            session.save()
            page = f"""
                <!doctype html>
                <html>
