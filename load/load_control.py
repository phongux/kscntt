...
def count_rows():
    ...
    con = connect.get_connection()
    cur = con.cursor()
    cur.execute(f"""select count(*) from nhap_kho""")
...
def get_rows(display, start_w):
    ...
    cur = con.cursor()
    cur.execute(
        f"""select id, ngay,so_phieu,ma_hang,ten_hang,don_vi_tinh, so_luong,don_gia,thanh_tien,ghi_chu,update_time from nhap_kho""")
    rows = cur.fetchall()
    con.commit()
    cur.close()
    con.close()
...
def application(environment, start_response):
    ...
    if 'username' not in session:
        ...
        if ps[0][2] > 0:
            ...
            page = '{"product":'
            objects_list = []
            cols = ["id", "ngay","so_phieu","ma_hang","ten_hang","don_vi_tinh", "so_luong","don_gia","thanh_tien","ghi_chu","update_time"]
