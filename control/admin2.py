...
def application(environment, start_response):
    ...
    head = config.module.head()
    headlink = config.module.headlink()
    menuadmin = config.module.menuadmin()
    menuuser = config.module.menuuser()
    menuhead = config.module.menuhead()
    menufoot = config.module.menufoot()
    save = config.module.save()
    load = config.module.load()
    # Get the session object from the environ
    session = environment['beaker.session']

    if 'username' not in session:
        page = config.login.login_again()
        ...
        con = config.conn.get_connection()
        cur = con.cursor()
        cur.execute(
            ...
            if int(ps[0][2]) == 2:
                ...
                page += f"""
                    ...
                    <ul class="nav nav-tabs">
                        <li class="active"><a href="{config.module.control}/account_manager">{table}</a></li>
                    </ul>
                    <h2>Table {table} </h2>
...
session_opts = json.loads(config.sess.session_opts())
